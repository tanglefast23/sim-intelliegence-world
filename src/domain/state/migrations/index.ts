...
export function migrateStateCopy(candidate: unknown, nextGenerationId: string): WorldState {
  const version = typeof candidate === 'object' && candidate !== null
    ...
    : undefined;
  if (version === 1) return migrateV4ToV5(migrateV3ToV4(migrateV2ToV3(migrateV1ToV2(candidate, nextGenerationId), nextGenerationId), nextGenerationId), nextGenerationId);
  if (version === 2) return migrateV4ToV5(migrateV3ToV4(migrateV2ToV3(candidate, nextGenerationId), nextGenerationId), nextGenerationId);
  if (version === 3) return migrateV4ToV5(migrateV3ToV4(candidate, nextGenerationId), nextGenerationId);
  if (version === 4) return migrateV4ToV5(candidate, nextGenerationId);
  if (version === 5) {
