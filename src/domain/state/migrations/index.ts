...
export function migrateStateCopy(candidate: unknown, nextGenerationId: string): WorldState {
  ...
  if (version === 1) {
    ...
    return migrateV6ToV7(migrateV5ToV6(v5, nextGenerationId), nextGenerationId);
  }
  if (version === 2) {
  ...
  if (version === 4) return migrateV6ToV7(migrateV5ToV6(migrateV4ToV5(candidate, nextGenerationId), nextGenerationId), nextGenerationId);
  if (version === 5) return migrateV6ToV7(migrateV5ToV6(candidate, nextGenerationId), nextGenerationId);
  if (version === 6) return migrateV6ToV7(candidate, nextGenerationId);
  if (version === 7) {
