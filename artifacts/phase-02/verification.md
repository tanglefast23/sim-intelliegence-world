...
The first hosted Linux run reached the packaged smoke and then failed because Chromium requires its Linux `chrome-sandbox` helper to be root-owned with mode `4755`. The next run proved that setup, then exposed a Chromium zygote failure caused by the space in the generated Linux package path. Linux packages now use the no-space application name `si-world`; macOS and Windows keep `SI World`. A third run reached the renderer but its Xvfb capture failed in Chromium Viz, so hosted smoke now calls Electron's pre-ready software-rendering API through a smoke-only environment flag. The CI workflow configures the exact generated helper in the ephemeral runner and tests that `--no-sandbox` is absent. Local macOS verification remained green with normal hardware acceleration.
